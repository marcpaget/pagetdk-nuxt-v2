...
CREATE VIEW "extensions"."pg_stat_statements_info" AS (SELECT pg_stat_statements_info.dealloc, pg_stat_statements_info.stats_reset FROM pg_stat_statements_info() pg_stat_statements_info(dealloc, stats_reset));--> statement-breakpoint
...
CREATE POLICY "Users can insert their own profile." ON "profiles" AS PERMISSIVE FOR INSERT TO public WITH CHECK ((( SELECT auth.uid() AS uid) = id));--> statement-breakpoint
...
CREATE POLICY "Anyone can upload an avatar." ON "storage"."objects" AS PERMISSIVE FOR INSERT TO public WITH CHECK ((bucket_id = 'avatars'::text));--> statement-breakpoint
