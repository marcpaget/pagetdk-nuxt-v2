...
export const customOauthProvidersInAuth = auth.table(
  'custom_oauth_providers',
  {
    ...
    updatedAt: timestamp('updated_at', { withTimezone: true })
      .default(sql`now()`)
      .notNull(),
    customClaimsAllowlist: text('custom_claims_allowlist')
      .array()
      .default([])
      .notNull(),
  },
...
export const keyInPgsodium = pgsodium.table(
  'key',
  {
    ...
    keyContext: customType({ dataType: () => 'bytea' })('key_context').default(
      "'\\x7067736f6469756d'",
    ),
    name: text(),
  ...
  (table) => [
    ...
    uniqueIndex('key_key_id_key_context_key_type_idx').using(
      'btree',
      table.keyId.asc().nullsLast(),
      table.keyContext.asc().nullsLast(),
      table.keyType.asc().nullsLast(),
    ),
    index('key_status_idx')
      .using('btree', table.status.asc().nullsLast())
      .where(
        sql`(status = ANY (ARRAY['valid'::pgsodium.key_status, 'default'::pgsodium.key_status]))`,
      ),
    uniqueIndex('key_status_idx1')
      .using('btree', table.status.asc().nullsLast())
      .where(sql`(status = 'default'::pgsodium.key_status)`),
    unique('pgsodium_key_unique_name').on(table.name),
    check('key_key_context_check', sql`(length(key_context) = 8)`),
    check(
      'pgsodium_raw',
      sql`
CASE
...
export const subscriptionInRealtime = realtime.table(
  ...
  (table) => [
    ...
    uniqueIndex(
      'subscription_subscription_id_entity_filters_action_filter_selec',
    ).using(
      'btree',
...
export const objectsInStorage = storage.table.withRLS(
  ...
  (table) => [
    ...
    pgPolicy('Anyone can upload an avatar.', {
      for: 'insert',
      withCheck: sql`(bucket_id = 'avatars'::text)`,
    }),

    pgPolicy('Avatar images are publicly accessible.', {
      for: 'select',
      using: sql`(bucket_id = 'avatars'::text)`,
    }),
  ],
)
...
export const pgStatStatementsInfoInExtensions = extensions
  ...
  .as(
    sql`SELECT pg_stat_statements_info.dealloc, pg_stat_statements_info.stats_reset FROM pg_stat_statements_info() pg_stat_statements_info(dealloc, stats_reset)`,
  )

export const decryptedKeyInPgsodium = pgsodium
  .view('decrypted_key', {
    id: uuid(),
    ...
    rawKey: customType({ dataType: () => 'bytea' })('raw_key'),
    decryptedRawKey: customType({ dataType: () => 'bytea' })(
      'decrypted_raw_key',
    ),
    rawKeyNonce: customType({ dataType: () => 'bytea' })('raw_key_nonce'),
    parentKey: uuid('parent_key'),
...
export const maskColumnsInPgsodium = pgsodium
  ...
  .as(
    sql`SELECT a.attname, a.attrelid, m.key_id, m.key_id_column, m.associated_columns, m.nonce_column, m.format_type FROM pg_attribute a LEFT JOIN pgsodium.masking_rule m ON m.attrelid = a.attrelid AND m.attname = a.attname WHERE a.attnum > 0 AND NOT a.attisdropped ORDER BY a.attnum`,
  )

export const maskingRuleInPgsodium = pgsodium
  .view('masking_rule', {
    attrelid: customType({ dataType: () => 'oid' })(),
    attnum: integer(),
...
export const validKeyInPgsodium = pgsodium
  ...
  .as(
    sql`SELECT key.id, key.name, key.status, key.key_type, key.key_id, key.key_context, key.created, key.expires, key.associated_data FROM pgsodium.key WHERE (key.status = ANY (ARRAY['valid'::pgsodium.key_status, 'default'::pgsodium.key_status])) AND CASE WHEN key.expires IS NULL THEN true ELSE key.expires > now() END`,
  )
